...
# creates a geojson with n random multipoints
...
# creates a geojson with n random points
def geojson_rdm_points(n=100):
    ...
    areafeature = []
    for i in range(n):
        areafeature.append(Feature(geometry=Point(coords_rdm(1, lat1, latx, long1, longx)[0]),
                            properties={"area": 1, "sensor": random.randrange(0, 2500)}))
        areafeature.append(Feature(geometry=Point(coords_rdm(1, lat2, latx, long1, longx)[0]), 
                            properties={"area": 2, "sensor": random.randrange(2500, 5000)}))
        areafeature.append(Feature(geometry=Point(coords_rdm(1, lat1, latx, long2, longx)[0]), 
                            properties={"area": 3, "sensor": random.randrange(5000, 7500)}))
        areafeature.append(Feature(geometry=Point(coords_rdm(1, lat2, latx, long2, longx)[0]), 
                            properties={"area": 4, "sensor": random.randrange(7500, 12200)}))
...
def read_myjson(json):  
	...
	for i in range(len(json["points"])):
		...
		data.append([json["points"][i]["long"], json["points"][i]["lat"], 
                     json["info"][i]["sensor_light"], json["info"][i]["timestamp"], json["device"]])
	return data
...
def geojson_pointfeature(data):
	...
	for i in range(len(data)):
		pointfeature.append(Feature(geometry=Point([data[i][0], data[i][1]]), 
                            properties={"sensor": data[i][2], "timestamp": data[i][3]}))
	return pointfeature
...
def search_db_time(starttime, endtime):
    ...
    datapoints = db.execute(f"SELECT * FROM location WHERE {where} ")
    return datapoints


# creates a geojson from data from sql
def db_to_geojson(sql):
    points = []
	# [[long, lat, sensor, timestamp, user],]
    ...
    return featurecollection_geral
